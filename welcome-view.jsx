...
function drawScene(ctx, W, H, p, tt, now) {
  ...
  // ============================================================================
  {
    ...
    const fig = (px, py, sc, kind, ph, opt) => {
      ...
      // opt.noArms: the caller draws its own arms (e.g. clapping/cheering
      // spectators) — skip fig's default pair so the figure isn't four-armed
      if (opt.noArms) { /* caller owns the arms */ } else {
      ...
      }   // end of the default-arms branch (opt.noArms skips it)
    ...
    if (dayA > 0.02) {
      ...
      const wEvening = _clamp((p - 0.50) / 0.08, 0, 1);                                          // evening waits for the day shift to wind down (0.32 made day+evening figures pile up together on the mid-bank — Hassan: "too crowded, don't know who is doing what")
      ...
      earth(dx, dyy + 8, 30);
      fig(dx - 12, dyy + 6, 1.50, 'hang', 1.2, { shirt: '#7c2f6b', hairStyle: 'braid' });
      fig(dx + 26, dyy + 8, 1.30, 'carry', 4.0, { shirt: '#5a7d3a', dir: -1 });   // brings the next fish TO the rack (the second 'hang' stood directly inside the first — Hassan)
      ...
      {
        const paX = W * 0.805, paY = H - 48, paR = 78;             // lifted off the bottom edge (the front rim was getting cut off at H-22)
        ...
        ctx.save();
        const clr = ctx.createRadialGradient(paX, paY, 2, paX, paY, paR * 1.26);
        ...
        const GR = paR * 1.26;                                     // ground ellipse wide enough for the wrap-around dancer ring
        ctx.fillStyle = clr; ctx.beginPath(); ctx.ellipse(paX, paY, GR, GR * 0.44, 0, 0, 6.283); ctx.fill();
        ctx.strokeStyle = `rgba(${Math.round(_lerp(120,58,nm))},${Math.round(_lerp(92,44,nm))},${Math.round(_lerp(52,26,nm))},0.85)`;
        ctx.lineWidth = 3; ctx.beginPath(); ctx.ellipse(paX, paY, GR, GR * 0.44, 0, 0, 6.283); ctx.stroke();
        ctx.restore();
        ...
        const spectator = (a, kind2, sc2, sh, hs, i, rf) => {
          ...
          fig(sx2, sy2 - bounce, sc2, 'idle', i * 2.1, { shirt: sh, hairStyle: hs, dir: facing, noArms: true });   // WE draw the arms (clap/cheer) — noArms stops fig's default pair, so no four-armed figures
          const shY = sy2 - bounce - 26 * sc2;                       // shoulder height
          const hand = (hx3, hy3) => { ctx.fillStyle = '#a3704a'; ctx.beginPath(); ctx.arc(hx3, hy3, 1.8 * sc2, 0, 6.283); ctx.fill(); };
          ctx.strokeStyle = '#a3704a'; ctx.lineWidth = 2.8 * sc2; ctx.lineCap = 'round';
          if (kind2 === 'clap') {
            ...
            // CHEERING (variant A): both arms high in a WIDE V, sweeping side
            // to side, jumping on the beat
            ...
            // CHEERING (variant B): FIST PUMP — one arm punches straight up on
            // the beat, the other hand rests on the hip
            const punch = onBeat * 7 * sc2;
            const p1x = sx2 + facing * 2 * sc2, p1y = shY - 8 * sc2 - punch;
            ctx.beginPath(); ctx.moveTo(sx2 + facing * 3 * sc2, shY); ctx.lineTo(p1x, p1y); ctx.stroke();
            hand(p1x, p1y);
            ctx.beginPath(); ctx.moveTo(sx2 - facing * 3 * sc2, shY);
            ctx.lineTo(sx2 - facing * 6 * sc2, shY + 6 * sc2); ctx.stroke();   // hand on hip
          }
        };

        // ---- PLANT RING first (farthest back — it was painting OVER a
        //   spectator when drawn last at the same radius as the back row):
        //   RICHER now — taller tuft clusters, flower heads, and cattail
        //   stands every third tuft. ----
        for (let pr2 = 0; pr2 < 9; pr2++) {
          const pa2 = 5.15 + pr2 * 0.30;                             // RIGHT + bottom-right arc only — the arbour owns the left, so plants there stabbed through its roof (Hassan: "hut colliding")
          ...
          for (let bl2 = 0; bl2 < 7; bl2++) {
            const bx3 = ppx + (bl2 - 3) * 2.8, bh3 = 10 + (bl2 % 3) * 5;
            const sw3 = Math.sin(tt * 1.5 + pr2 + bl2 * 0.6) * 2.2;
            ctx.beginPath(); ctx.moveTo(bx3, ppy); ctx.quadraticCurveTo(bx3 + sw3 * 0.5, ppy - bh3 * 0.6, bx3 + sw3, ppy - bh3); ctx.stroke();
          }
          if (pr2 % 3 === 0) {
            // a cattail stand: two tall stems with brown seed heads
            for (let ct2 = 0; ct2 < 2; ct2++) {
              const cx3 = ppx + 8 + ct2 * 5, sw4 = Math.sin(tt * 1.1 + pr2 + ct2) * 2;
              ctx.strokeStyle = `rgba(${Math.round(_lerp(88,44,nm))},${Math.round(_lerp(130,66,nm))},${Math.round(_lerp(56,30,nm))},0.95)`;
              ctx.lineWidth = 1.2;
              ctx.beginPath(); ctx.moveTo(cx3, ppy); ctx.quadraticCurveTo(cx3 + sw4 * 0.5, ppy - 12, cx3 + sw4, ppy - 22); ctx.stroke();
              ctx.fillStyle = `rgb(${Math.round(_lerp(120,62,nm))},${Math.round(_lerp(78,44,nm))},${Math.round(_lerp(40,24,nm))})`;
              ctx.beginPath(); ctx.ellipse(cx3 + sw4, ppy - 24, 1.4, 4, 0, 0, 6.283); ctx.fill();
            }
          } else {
            ctx.fillStyle = `rgba(${Math.round(_lerp(226,124,nm))},${Math.round(_lerp(168,90,nm))},${Math.round(_lerp(90,50,nm))},0.95)`;
            ctx.beginPath(); ctx.arc(ppx + Math.sin(tt + pr2) * 2, ppy - 16, 1.6, 0, 6.283); ctx.fill();
            ctx.beginPath(); ctx.arc(ppx - 5 + Math.sin(tt + pr2 + 2) * 2, ppy - 12, 1.3, 0, 6.283); ctx.fill();
          }
        }
        ...
        // ---- FOUR-DIRECTIONS FLAGS around the rim (drawn before the dancers) --
        [[5.65, '#d4a017'], [0.55, '#b8351e'], [4.15, '#2a241e'], [6.15, '#e8dcc0']].forEach(([fa2, fc], fi) => {
          const fxp2 = paX + Math.cos(fa2) * paR * 1.14, fyp2 = paY + Math.sin(fa2) * paR * 0.5;
          ctx.strokeStyle = nd('#5a3a1c'); ctx.lineWidth = 1.8; ctx.lineCap = 'round';
          ctx.beginPath(); ctx.moveTo(fxp2, fyp2); ctx.lineTo(fxp2, fyp2 - 30); ctx.stroke();
          const flut = Math.sin(tt * 2.2 + fi * 1.7) * 2.5;                      // banner flutters
          ctx.fillStyle = nd(fc);
          ctx.beginPath();
          ctx.moveTo(fxp2, fyp2 - 30); ctx.quadraticCurveTo(fxp2 + 6, fyp2 - 29 + flut * 0.4, fxp2 + 11, fyp2 - 27 + flut);
          ctx.lineTo(fxp2 + 10, fyp2 - 22 + flut); ctx.quadraticCurveTo(fxp2 + 5, fyp2 - 24 + flut * 0.4, fxp2, fyp2 - 24);
          ctx.closePath(); ctx.fill();
        });

        // ---- THE ARBOUR — CENTRED in the arena, drawn between the back and
        //   front dancers so the dance ring genuinely wraps AROUND it (this is
        //   the real layout: drum + singers at the centre, dancers circling).
        //   The host drum sits ELEVATED on a carved wooden stand. ----
        const drawArbour = () => {
          const abX = paX, base = paY + 4;                          // centre of the arena
          const RW = 58;                                            // roof half-width (dancers circle OUTSIDE this)
          const eaveY = base - 78;                                  // tall — clears every head
          // warm glow from inside
          const ag = ctx.createRadialGradient(abX, base - 22, 6, abX, base - 22, 74);
          ag.addColorStop(0, `rgba(255,190,110,${0.30 + 0.16 * nm})`);
          ag.addColorStop(1, 'rgba(255,190,110,0)');
          ctx.fillStyle = ag; ctx.beginPath(); ctx.ellipse(abX, base - 14, 70, 40, 0, 0, 6.283); ctx.fill();
          // BACK posts
          ctx.strokeStyle = nd('#6b4a26'); ctx.lineWidth = 3; ctx.lineCap = 'round';
          [[-40, -12], [0, -15], [40, -12]].forEach(([px2, py2]) => {
            ctx.beginPath(); ctx.moveTo(abX + px2, base + py2); ctx.lineTo(abX + px2 * 0.9, eaveY + 7); ctx.stroke();
          });
          // back two singers (seated on the ground, behind the drum)
          const singerCols = ['#c93a1e', '#1f4e8f', '#5a7d3a', '#7c2f6b'];
          fig(abX - 22, base - 10, 1.12, 'sit', 0.4, { shirt: singerCols[0], hairStyle: 'braid', dir: 1 });
          fig(abX + 22, base - 10, 1.12, 'sit', 2.1, { shirt: singerCols[1], hairStyle: 'short', dir: -1 });
          // ---- THE ELEVATED DRUM on a carved ash-wood stand ----
          const dcx = abX, dcy = base - 22, drumR = 18 + beat * 1.4;
          // stand: curved side-supports + a foot on the ground (like the photo)
          ctx.strokeStyle = nd('#d8bd86'); ctx.lineWidth = 4.5; ctx.lineCap = 'round';
          for (const sgn of [-1, 1]) {
            ctx.beginPath(); ctx.moveTo(dcx + sgn * drumR * 0.55, dcy + drumR * 0.4);
            ctx.quadraticCurveTo(dcx + sgn * drumR * 1.05, base - 4, dcx + sgn * drumR * 0.55, base + 3); ctx.stroke();
          }
          ctx.lineWidth = 3; ctx.beginPath(); ctx.moveTo(dcx - drumR * 0.6, base + 3); ctx.lineTo(dcx + drumR * 0.6, base + 3); ctx.stroke();
          // drum shell (coloured band + lacing, like the reference)
          ctx.fillStyle = nd('#7a2a2a'); ctx.beginPath(); ctx.ellipse(dcx, dcy, drumR, drumR * 0.52, 0, 0, 6.283); ctx.fill();
          for (let bnd = 0; bnd < 5; bnd++) {                       // stripes around the shell
            ctx.strokeStyle = nd(['#e2b81e', '#1f7fbf', '#2f9f4f', '#e2721e', '#eae0c8'][bnd]); ctx.lineWidth = 1.3;
            ctx.beginPath(); ctx.ellipse(dcx, dcy + 2 + bnd * 1.3, drumR * (1 - bnd * 0.02), drumR * 0.5 * (1 - bnd * 0.02), 0, 0.15, Math.PI - 0.15); ctx.stroke();
          }
          ctx.fillStyle = `rgba(${Math.round(_lerp(236,136,nm))},${Math.round(_lerp(214,116,nm))},${Math.round(_lerp(170,84,nm))},1)`;   // pale hide head
          ctx.beginPath(); ctx.ellipse(dcx, dcy - 3, drumR * 0.94, drumR * 0.46, 0, 0, 6.283); ctx.fill();
          ctx.strokeStyle = 'rgba(70,42,20,0.5)'; ctx.lineWidth = 1.4;   // lacing pegs down the shell
          for (let s = 0; s < 8; s++) { const a = -0.2 + s * (Math.PI + 0.4) / 7; const ex = dcx + Math.cos(a) * drumR * 0.92, ey = dcy - 3 + Math.sin(a) * drumR * 0.46; ctx.beginPath(); ctx.moveTo(ex, ey); ctx.lineTo(ex, ey + drumR * 0.4); ctx.stroke(); }
          // front two singers (in front of the drum)
          fig(abX - 22, base + 8, 1.12, 'sit', 3.3, { shirt: singerCols[2], hairStyle: 'long', dir: 1 });
          fig(abX + 22, base + 8, 1.12, 'sit', 1.2, { shirt: singerCols[3], hairStyle: 'braid', dir: -1 });
          // each singer's beater reaches UP to the elevated drum head on the beat
          [[-22, -10], [22, -10], [-22, 8], [22, 8]].forEach(([ox, oy], i) => {
            const lift = Math.abs(Math.sin(tt * 3.4 + i * 0.3)) * 4;
            const hx = dcx + (ox > 0 ? -1 : 1) * drumR * 0.4, hy = dcy + 1 - lift;
            ctx.strokeStyle = '#a3704a'; ctx.lineWidth = 2; ctx.lineCap = 'round';
            ctx.beginPath(); ctx.moveTo(abX + ox * 0.8, base + oy - 12); ctx.lineTo(hx, hy + 3); ctx.stroke();   // forearm
            ctx.strokeStyle = nd('#4a2e14'); ctx.lineWidth = 1.6;
            ctx.beginPath(); ctx.moveTo(hx, hy + 3); ctx.lineTo(hx + (ox > 0 ? -3 : 3), hy - 4); ctx.stroke();   // beater
            ctx.fillStyle = nd('#d8c8a0'); ctx.beginPath(); ctx.arc(hx + (ox > 0 ? -3 : 3), hy - 4, 1.6, 0, 6.283); ctx.fill();
          });
          // FRONT posts + braces
          ctx.strokeStyle = nd('#6b4a26'); ctx.lineWidth = 3.4; ctx.lineCap = 'round';
          [[-58, 8], [58, 8]].forEach(([px2, py2]) => {
            ctx.beginPath(); ctx.moveTo(abX + px2, base + py2); ctx.lineTo(abX + px2 * 0.92, eaveY + 9); ctx.stroke();
            ctx.lineWidth = 2; ctx.beginPath(); ctx.moveTo(abX + px2 * 0.7, base + py2 - 8); ctx.lineTo(abX + px2 * 0.98, eaveY + 16); ctx.stroke();
            ctx.lineWidth = 3.4;
          });
          // DOMED ROOF
          ctx.fillStyle = `rgb(${Math.round(_lerp(222,116,nm))},${Math.round(_lerp(196,98,nm))},${Math.round(_lerp(150,68,nm))})`;
          ctx.beginPath();
          ctx.moveTo(abX - RW, eaveY + 11); ctx.quadraticCurveTo(abX, eaveY - 40, abX + RW, eaveY + 11);
          ctx.quadraticCurveTo(abX, eaveY + 1, abX - RW, eaveY + 11); ctx.closePath(); ctx.fill();
          ctx.strokeStyle = nd('#8a6636'); ctx.lineWidth = 2.2;
          ctx.beginPath(); ctx.moveTo(abX - RW, eaveY + 11); ctx.quadraticCurveTo(abX, eaveY + 1, abX + RW, eaveY + 11); ctx.stroke();
          ctx.strokeStyle = nd('#9a7440'); ctx.lineWidth = 1.2;
          for (let rf2 = -5; rf2 <= 5; rf2++) {
            ctx.beginPath(); ctx.moveTo(abX + rf2 * 10.8, eaveY + 9 - Math.abs(rf2) * 0.5); ctx.lineTo(abX + rf2 * 2.6, eaveY - 24); ctx.stroke();
          }
        };

        // ---- DANCERS circling the arbour in bright regalia — EIGHT men in
        //   full bustle-and-roach fancy dress, incl. a LITTLE ONE. Drawn in TWO
        //   passes (back ring, then arbour, then front ring) so they wrap it. --
        const REG = [['#e23a1e', '#f2b81e'], ['#1f7fbf', '#f2d21e'], ['#8f2f6b', '#e2721e'], ['#2f9f4f', '#e23a7b'], ['#e2901e', '#2f5ecf'], ['#c9401e', '#2fbf9f'], ['#3f6fdf', '#f2a01e'], ['#9f3f1e', '#f2e21e']];
        const SCL = [1.24, 1.22, 1.26, 1.24, 1.22, 0.94, 1.24, 1.2];
        const dN = 8, spin = tt * 0.32;
        const dancers = [];
        for (let d = 0; d < dN; d++) {
          const a = spin + d * (Math.PI * 2 / dN);
          dancers.push({ x: paX + Math.cos(a) * paR * 1.06, y: paY + Math.sin(a) * paR * 0.5, a, d });   // ring OUTSIDE the arbour roof
        }
        const drawDancer = ({ x, y, a, d }) => {
          const scd = SCL[d];
          const ddir = Math.cos(a + 0.35) >= 0 ? 1 : -1;            // face the direction of travel
          const step = Math.abs(Math.sin(tt * 3.4 + d * 1.1));      // toe-heel on the beat
          const hop = step * 2.6;
          const fy = y - hop;
          const c1 = nd(REG[d][0]), c2 = nd(REG[d][1]), tip = nd('#3a2410');
          if (step < 0.3) {                                         // dust as the foot lands
            ctx.fillStyle = `rgba(${Math.round(_lerp(190,90,nm))},${Math.round(_lerp(168,78,nm))},${Math.round(_lerp(130,52,nm))},${0.30 * (1 - step / 0.3)})`;
            ctx.beginPath(); ctx.ellipse(x - ddir * 3, y + 1, 5 + (1 - step / 0.3) * 3, 1.6, 0, 0, 6.283); ctx.fill();
          }
          const bx = x - ddir * 2, by = fy - 14 * scd;              // bustle behind the back
          ...
          fig(x, fy, scd, 'dance', d * 1.1, { shirt: REG[d][0], hairStyle: 'short', dir: ddir });
          const headTop = fy - (scd * 1.3) * 36.5;                  // roach on the crown
        ...
        dancers.filter(dd => dd.y < paY).sort((A, B) => A.y - B.y).forEach(drawDancer);   // BACK ring (behind the arbour)
        drawArbour();
        ...
        spectator(6.05, 'sitfam', 1.35, '#1f4e8f', 'long', 5, 1.7);
        {
          const kx = paX + paR * 2.0, ky = paY + paR * 0.11;
          const khop = Math.abs(Math.sin(tt * 3.4 + 0.6)) * 2.4;
          fig(kx, ky - khop, 0.98, 'dance', 2.6, { shirt: '#2f9f4f', hairStyle: 'short', dir: -1 });   // BIGGER kid (Hassan)
        }
      ...
      if (isEvening) {
        const stX = W * 0.56, stY = ground(stX) + 6;   // its OWN open ground — fx-50 sat right on top of the chop/fire/smokehouse cluster
      ...
      if (isEvening) {                                          // evening ONLY — at midday it crowded the fishing pair + dice on the same stretch
      ...
      const drawHorse = (hx, hy, sc, ph, headDown) => {
        ...
        const leg = (lx, swing) => {
          ...
          ctx.quadraticCurveTo(lx + swing * 0.4, hy + 7 * sc, lx + swing, hy + 14 * sc);
          ctx.stroke();
          // hoof
          ctx.strokeStyle = 'rgba(20,14,8,1)'; ctx.lineWidth = 2.8 * sc;
          ctx.beginPath(); ctx.moveTo(lx + swing, hy + 13.5 * sc); ctx.lineTo(lx + swing, hy + 15 * sc); ctx.stroke();
        };
        ...
        const headX = hx - 22 * sc, headY = hy - 13 * sc + t * 16 * sc;      // head carried HIGH, drops to graze
        ctx.fillStyle = coat;
        ctx.beginPath();
      ...
      {
        ctx.save(); ctx.globalAlpha = dayA;
        const sdx = W * 0.61, sdy = ground(W * 0.61) + 44;   // moved off the 0.505 stretch — it stacked onto the fishing pair + talking circle
      ...
      // broad level back, (2) the caught fish is gripped in the JAWS as the head
      // lifts (it used to float up from the paw). Day-only: fades before dusk.
      if (bearFade > 0.01) {
      ...
      const furHi = 'rgba(120,84,48,0.45)';                      // soft back sheen (no hump bulge)
      ...
      ctx.quadraticCurveTo(-2 * S, -7.4 * S, 4 * S, -6.6 * S);                    // broad level back (hump flattened)
      ...
      // hump highlight
      ctx.fillStyle = furHi;
      ctx.beginPath(); ctx.ellipse(-1 * S, -6.2 * S, 6 * S, 1.4 * S, -0.1, 0, 6.283); ctx.fill();
      // belly shadow
    ...
    // ---- EVENING PACK-UP — TOP-LEVEL, no longer trapped inside the nightA
    //   gate. It used to require nightA>0.02 AND evA>0.04 (a sliver at nm
    //   .62-.65), so through the whole red-sky window the bank stood EMPTY —
    //   that was the "nothing happening at dusk" dead zone. ----
    {
      const evA = _clamp((nm - 0.30) / 0.08, 0, 1) * (1 - _clamp((nm - 0.60) / 0.10, 0, 1));   // owns the red-sky window; hands to the night circle at true dark
      if (evA > 0.04) {
        ...
        // EVENING STICK-DICE by the packing racks — the day's last game before
        // the fire circle forms (more life in the red-sky window)
        {
    ...
    if (nightA > 0.02) {
      ...
      // (the EVENING PACK-UP moved to its own top-level gate above — inside this
      //  nightA block it could only ever appear for a sliver at nm .62-.65,
      //  leaving the whole red-sky window empty)
      ...
      // BEADWORK BY FIRELIGHT — a woman near the sleeping children works beads
      // onto a bandolier strap by a tiny work-fire (quill- and beadwork were
      // night-and-winter work). Her needle hand rises and dips; beads glint.
      {
        const bwX = fx + 138, bwY = ground(fx + 138) + 6;
        const bwf = 0.8 + Math.sin(tt * 7.7) * 0.12;                              // small flame breath
        const bwp = ctx.createRadialGradient(bwX - 14, bwY - 2, 1, bwX - 14, bwY - 2, 30 * bwf);
        bwp.addColorStop(0, 'rgba(255,160,70,0.28)'); bwp.addColorStop(1, 'rgba(255,160,70,0)');
        ctx.fillStyle = bwp;
        ctx.beginPath(); ctx.ellipse(bwX - 6, bwY, 36 * bwf, 13 * bwf, 0, 0, 6.283); ctx.fill();
        ctx.fillStyle = 'rgba(255,185,85,0.95)';                                  // the work-fire
        ctx.beginPath(); ctx.ellipse(bwX - 14, bwY - 4, 2.0, 3.8 * bwf, 0, 0, 6.283); ctx.fill();
        ctx.fillStyle = 'rgba(255,240,180,0.9)';
        ctx.beginPath(); ctx.ellipse(bwX - 14, bwY - 3, 0.9, 1.8 * bwf, 0, 0, 6.283); ctx.fill();
        fig(bwX, bwY, 1.3, 'sit', 0.9, { shirt: '#7c2f6b', hairStyle: 'braid', dir: -1 });
        // the strap across her lap + the needle hand rising and dipping
        ctx.strokeStyle = 'rgb(150,116,72)'; ctx.lineWidth = 2.2; ctx.lineCap = 'round';
        ctx.beginPath(); ctx.moveTo(bwX - 9, bwY - 8); ctx.lineTo(bwX + 8, bwY - 6); ctx.stroke();
        const stitch = Math.abs(Math.sin(tt * 2.2));
        ctx.strokeStyle = '#a3704a'; ctx.lineWidth = 1.8;
        ctx.beginPath(); ctx.moveTo(bwX - 4, bwY - 16); ctx.lineTo(bwX - 7 + stitch * 3, bwY - 9 - stitch * 5); ctx.stroke();
        // beads glinting on the strap in the firelight
        const beadCols = ['rgba(120,180,255,0.9)', 'rgba(255,120,90,0.9)', 'rgba(255,220,120,0.9)'];
        for (let bd = 0; bd < 5; bd++) {
          ctx.fillStyle = beadCols[bd % 3];
          ctx.globalAlpha = 0.5 + 0.5 * Math.abs(Math.sin(tt * 3 + bd * 1.7));
          ctx.beginPath(); ctx.arc(bwX - 8 + bd * 3.6, bwY - 7.6 + bd * 0.4, 0.8, 0, 6.283); ctx.fill();
        }
        ctx.globalAlpha = 1;
      }
      ...
      // (2) NIGHT SUPPER — a family shares the evening meal around a small fire:
      //     everyone seated with a bowl, lifting food to their mouth on a gentle
      //     cycle. (Replaces the moccasin game + flute — their single-line arms
      //     read as stiff "sticks"; Hassan wanted natural activities like eating.
      //     Here arms are drawn BENT at the elbow so they read as arms.) ----
      {
        const msx = W * 0.52, msy = ground(W * 0.52) + 34;
        // the cook-fire + warm pool of light
        const flk = 0.8 + Math.sin(tt * 8.5) * 0.12 + Math.sin(tt * 21) * 0.06;
        const gp = ctx.createRadialGradient(msx, msy - 3, 2, msx, msy - 3, 60 * flk);
        gp.addColorStop(0, 'rgba(255,160,70,0.34)'); gp.addColorStop(1, 'rgba(255,160,70,0)');
        ctx.fillStyle = gp; ctx.beginPath(); ctx.ellipse(msx, msy, 66 * flk, 22 * flk, 0, 0, 6.283); ctx.fill();
        // a diner: seated figure (noArms) + a BENT arm lifting a bowl to the mouth
        const diner = (dx0, dy0, sc, dir, sh, hs, phase) => {
          fig(dx0, dy0, sc, 'sit', phase, { shirt: sh, hairStyle: hs, dir, noArms: true });
          const eat = Math.sin(tt * 1.4 + phase) * 0.5 + 0.5;                       // 0 low, 1 at the mouth
          const shX = dx0 + dir * 3.2 * sc, shY = dy0 - 40 * (sc / 1.3);            // shoulder
          const hX = dx0 + dir * 5 * sc, hY = dy0 - (20 + eat * 9) * (sc / 1.3);    // hand rises to mouth
          const eX = dx0 + dir * 8 * sc, eY = dy0 - 26 * (sc / 1.3);                // elbow OUT (bend)
          ctx.strokeStyle = '#a3704a'; ctx.lineWidth = 2.6 * (sc / 1.3); ctx.lineCap = 'round';
          ctx.beginPath(); ctx.moveTo(shX, shY); ctx.lineTo(eX, eY); ctx.stroke();  // upper arm → elbow
          ctx.beginPath(); ctx.moveTo(eX, eY); ctx.lineTo(hX, hY); ctx.stroke();    // forearm → hand
          ctx.fillStyle = 'rgb(70,46,26)'; ctx.beginPath(); ctx.ellipse(hX, hY, 3.2 * (sc / 1.3), 1.9 * (sc / 1.3), 0, 0, 6.283); ctx.fill();  // bowl
          ctx.fillStyle = 'rgba(214,186,132,0.95)'; ctx.beginPath(); ctx.ellipse(hX, hY - 0.5, 2.2 * (sc / 1.3), 1.1 * (sc / 1.3), 0, 0, 6.283); ctx.fill();  // food
        };
        // back row (drawn first), then the fire + pot, then the front row
        diner(msx - 20, msy - 9, 1.25, 1, '#1f4e8f', 'short', 0.4);
        diner(msx + 20, msy - 9, 1.28, -1, '#7c2f6b', 'braid', 2.1);
        // the fire + a cook-pot on a low tripod at the centre
        ctx.strokeStyle = 'rgb(52,32,16)'; ctx.lineWidth = 1.4; ctx.lineCap = 'round';
        ctx.beginPath(); ctx.moveTo(msx - 7, msy + 1); ctx.lineTo(msx, msy - 12); ctx.stroke();
        ctx.beginPath(); ctx.moveTo(msx + 7, msy + 1); ctx.lineTo(msx, msy - 12); ctx.stroke();
        ctx.fillStyle = 'rgb(44,38,34)'; ctx.beginPath(); ctx.ellipse(msx, msy - 6, 5, 3.4, 0, 0, Math.PI); ctx.fill(); ctx.fillRect(msx - 5, msy - 7.5, 10, 2);  // pot
        ctx.fillStyle = `rgba(255,${Math.round(170 + 30 * flk)},80,0.95)`; ctx.beginPath(); ctx.ellipse(msx, msy + 1, 3.2, 5 * flk, 0, 0, 6.283); ctx.fill();   // flame licking up
        ctx.fillStyle = 'rgba(255,240,180,0.9)'; ctx.beginPath(); ctx.ellipse(msx, msy + 2, 1.4, 2.6 * flk, 0, 0, 6.283); ctx.fill();
        puffSmoke(msx, msy - 12, 0.28, nightA * 0.4);                               // supper steam
        diner(msx - 15, msy + 10, 1.3, 1, '#c93a1e', 'long', 3.3);
        diner(msx + 15, msy + 10, 1.26, -1, '#5a7d3a', 'short', 1.2);
        diner(msx + 1, msy + 15, 0.9, 1, '#d68a1f', 'short', 5.0);                  // a child, closest to us
      }
